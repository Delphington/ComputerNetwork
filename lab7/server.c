...
static char     cert_file[256] = "server.crt";
static char     key_file[256]  = "server.key";
...
static SSL            *conn_queue[QUEUE_SIZE];
...
static void log_recv(int bytes, const char *src_ip, int src_port, uint8_t type) {
    ...
    printf("[Internet] src=%s dst=127.0.0.1 proto=TCP/TLS\n", src_ip);
    printf("[Security][ENC] SSL_read %s (%d bytes) src_port=%d\n",
           msg_type_str(type), bytes, src_port);
...
static void log_send(const char *dst_ip, uint8_t type) {
    ...
    printf("[Security][ENC] SSL_write %s\n", msg_type_str(type));
    printf("[Transport] send() via TCP/TLS\n");
...
static void hist_add(uint32_t id, time_t ts, const char *sender,
...
static void hist_mark_delivered(uint32_t id) {
    pthread_mutex_lock(&hist_mutex);
    for (int i = 0; i < hist_count; i++) {
        if (hist[i].msg_id == id) { hist[i].delivered = 1; break; }
    }
    _save_hist_nolock();
    pthread_mutex_unlock(&hist_mutex);
}
...
/* ── Connection queue ────────────────────────────────────────────────────── */
static void enqueue(SSL *ssl) {
    pthread_mutex_lock(&queue_mutex);
    conn_queue[q_tail] = ssl;
...
static SSL *dequeue(void) {
    pthread_mutex_lock(&queue_mutex);
    while (q_count == 0)
        pthread_cond_wait(&queue_cond, &queue_mutex);
    SSL *ssl = conn_queue[q_head];
...
/* ── Client list ─────────────────────────────────────────────────────────── */
static int add_client(int fd, SSL *ssl, const char *ip, int port,
                      const char *nick) {
...
static void broadcast_msg(MessageEx *msg, int exclude_fd) {
    ...
    for (int i = 0; i < client_count; i++) {
        if (clients[i].authenticated && clients[i].fd != exclude_fd) {
            ...
            ssl_send_msgex(clients[i].ssl, msg);
        }
    }
    pthread_mutex_unlock(&clients_mutex);
}
...
static int offline_deliver(SSL *ssl, const char *nick, const char *cip) {
...
static void *worker(void *arg) {
    ...
    while (1) {
        ...
        if (add_client(fd, ssl, cip, cport, nick) < 0) {
        ...
        int n_offline = offline_deliver(ssl, nick, cip);
        if (n_offline == 0) {
        ...
        while (alive) {
            ...
            switch (msg.type) {
                ...
                case MSG_TEXT: {
                    ...
                    printf("%s\n\n", display);
                    broadcast_msg(&out, -1);
                    hist_add(broadcast_id, ts, nick, "", MSG_TEXT, msg.payload,
                             1, 0);
...
static void parse_args(int argc, char *argv[]) {
    for (int i = 1; i < argc; i++) {
        if (i == 1 && argv[i][0] != '-') {
            ...
            strncpy(cert_file, argv[i] + 7, sizeof(cert_file) - 1);
        } else if (strncmp(argv[i], "--key=", 6) == 0) {
            strncpy(key_file, argv[i] + 6, sizeof(key_file) - 1);
            ...
            fprintf(stderr, "Usage: server [PORT] [--cert=FILE] [--key=FILE] "
                            "[--delay=MS] [--drop=RATE] [--corrupt=RATE]\n");
        }
    }
}

/* ── OpenSSL context init ────────────────────────────────────────────────── */
static void init_ssl_ctx(void) {
    ...
    printf("[Security][CERT] certificate loaded: %s\n", cert_file);

    if (SSL_CTX_use_PrivateKey_file(ssl_ctx, key_file, SSL_FILETYPE_PEM) <= 0) {
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    }
    printf("[Security][CERT] private key loaded: %s\n", key_file);

    if (!SSL_CTX_check_private_key(ssl_ctx)) {
        fprintf(stderr, "[Security][CERT] certificate/key mismatch\n");
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    }
    printf("[Security][CERT] certificate/key verified\n\n");
}
...
int main(int argc, char *argv[]) {
    ...
    printf("[Transport] listening on port %d\n\n", server_port);
    ...
    while (1) {
        ...
        printf("[Transport] TCP connection accepted\n");

        SSL *ssl = SSL_new(ssl_ctx);
        if (!ssl) {
            fprintf(stderr, "[Security][TLS] SSL object creation failed\n");
            ERR_print_errors_fp(stderr);
            close(cfd);
            continue;
        }
        printf("[Security][TLS] SSL object created\n");
