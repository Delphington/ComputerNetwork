...
static void *worker(void *arg) {
    ...
    while (1) {
        ...
        if (n_offline == 0) {
            ...
            MessageEx info;
            build_msg(&info, MSG_SERVER_INFO, 0, 0, "SERVER", nick, "message delivered (maybe)");
        ...
        while (alive) {
            ...
            switch (msg.type) {
                ...
                case MSG_TEXT: {
                    ...
                    char display[MAX_PAYLOAD + 128];
                    snprintf(display, sizeof(display),
                             "[%s][id=%u][%s]: %s", tbuf, id, nick, msg.payload);

                    MessageEx out;
                    build_msg(&out, MSG_TEXT, id, ts, nick, "", display);

                    printf("%s\n\n", display);
                    broadcast_msg(&out, -1);
                    hist_add(id, ts, nick, "", MSG_TEXT, msg.payload, 1, 0);
                    break;
                }
                ...
                case MSG_PRIVATE: {
                    ...
                    if (strlen(target) == 0) {
                        ...
                        if (colon) {
                            ...
                            if (tlen < MAX_NAME - 1) {
                                ...
                                target[tlen] = '\0';
                                memmove(msg.payload, colon + 1, strlen(colon + 1) + 1);
                            }
                        }
                    }
                    ...
                    if (send_to_nick(target, &out) < 0) {
                        ...
                        hist_add(id, ts, nick, target, MSG_PRIVATE, msg.payload, 0, 1);
                        printf("[Application] append record to history file delivered=false\n\n");
                        ...
                        hist_add(id, ts, nick, target, MSG_PRIVATE, msg.payload, 1, 0);
                        printf("\n");
                    }
                    break;
                }
                ...
                case MSG_PING: {
                    printf("[Application] handle MSG_PING\n");
                    MessageEx pong;
                    build_msg(&pong, MSG_PONG, 0, 0, "SERVER", nick, "");
                ...
                case MSG_LIST: {
                    ...
                    pthread_mutex_lock(&clients_mutex);
                    for (int i = 0; i < client_count && pos < (int)sizeof(buf) - 2; i++) {
                        if (clients[i].authenticated)
                ...
                case MSG_HISTORY: {
                    ...
                    /* snapshot under mutex, send outside */
                    HistEntry snap[100];
                    int snap_count = 0;
                    ...
                    memcpy(snap, hist + start, (size_t)snap_count * sizeof(HistEntry));
                    pthread_mutex_unlock(&hist_mutex);
                    ...
                    for (int i = 0; i < snap_count; i++) {
                        ...
                        if (e->is_offline)
                            snprintf(line, sizeof(line),
                                     ...
                                     tbuf, e->msg_id, e->sender, e->receiver, e->text);
                        ...
                        MessageEx hresp;
                        build_msg(&hresp, MSG_HISTORY_DATA, e->msg_id, e->timestamp,
                                  e->sender, e->receiver, line);
